...
// export default function NavbaAdmin() {
...
export default function NavbaAdmin() {
  ...
  // ====== Fetch user data (original logic preserved) ======
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        setUser(currentUser);
  ...
  const handleProfileClick = () => router.push('/profileAdmin');
  const handleLogin = () => router.push('/login'); // Arahkan ke halaman login
  const handleLogout = async () => {
    try {
      await signOut(auth); // Logout dari Firebase
      router.push('/'); // Arahkan ke halaman utama
  ...
  // ====== Variants for premium interactions ======
  ...
  return (
    <nav className="relative z-50">
          ...
          >
            ...
            <div className="flex items-center">
              ...
              >
                <div className="relative">
                  {isOpen ? <FiX className="h-6 w-6" /> : <FiMenu className="h-6 w-6" />}
                  {/* tiny glow on hover */}
                  <span className="pointer-events-none absolute -inset-2 rounded-2xl opacity-0 group-hover:opacity-100 transition" />
                </div>
              </button>
            </div>
            ...
            <div className="flex items-center space-x-3">
              {loading ? (
                ...
                <div className="h-8 w-8 rounded-xl bg-gradient-to-r from-white/50 to-white/30 dark:from-white/10 dark:to-white/5 backdrop-blur animate-pulse" />
              ) : user ? (
                <>
                  <button
                    onClick={handleProfileClick}
      ...
      {/* ===== Mobile Menu (original structure preserved) ===== */}
      <AnimatePresence>
        {isOpen && (
          <>
            {/* Overlay */}
            ...
            >
              ...
              <div className="px-4 sm:px-6 py-4">
                <div className="grid sm:grid-cols-2 gap-3">
                  {/* Links block (kept as <a href> to preserve original structure) */}
                  ...
                  {/* Profile Section (original logic preserved) */}
                  <AnimatePresence>
                    {user && (
                      <motion.div
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="pt-2">
                            <button
                              ...
                              className="
                                inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold
                                border border-white/30 bg-black/50 dark:bg-black/10 backdrop-blur
                                hover:bg-black/70 dark:hover:bg-white/15 transition
                              "
                            >
