...
export default function UserDashboard() {
  ...
  const handleRating = async (workerId: string, rating: number) => {
    setSelectedRatings((prev) => ({ ...prev, [workerId]: rating }));
    await setDoc(doc(db, 'ratings', workerId), { uid: workerId, rating });
  };
  ...
  const handleChooseWorker = (worker: WorkerProfile) => setSelectedWorker(worker);
    ...
    try {
      ...
      const interval = setInterval(async () => {
        ...
        if (status === 'settlement' || status === 'capture') {
          ...
          await updateDoc(doc(db, 'payments', orderId), { status: 'success' });
          await updateDoc(doc(db, 'tugasdariuser', taskId), { statusPembayaran: 'success' });
  ...
  const getTugas = async () => {
    const querySnapshot = await getDocs(collection(db, 'tugasdariuser'));
    const tugasData = querySnapshot.docs.map((d) => ({ id: d.id, ...(d.data() as any) }));
    setTasks((tugasData as unknown) as Task[]);
  };
  ...
  return (
    <div
      data-no-x-scroll
      className="min-h-svh flex flex-col overflow-x-clip max-w-[100vw] bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 antialiased"
    >
      <main role="main" className="relative flex-1 overflow-x-clip">
        {/* Decorative layers, dipotong agar tak bikin overflow */}
        <div className="pointer-events-none absolute inset-0 overflow-hidden opacity-30">
          <div className="absolute -top-1/4 -left-1/4 h-[60vmax] w-[60vmax] rounded-full bg-gradient-to-tr from-blue-500 via-cyan-400 to-indigo-500 blur-3xl" />
          <div className="absolute -bottom-1/4 -right-1/4 h-[55vmax] w-[55vmax] rounded-full bg-gradient-to-tr from-violet-500 via-fuchsia-400 to-amber-300 blur-3xl" />
        </div>
        ...
        <div className="relative flex-1 overflow-x-clip">
          ...
          <div className="mx-auto w-full max-w-screen-2xl px-3 sm:px-6 lg:px-8 py-4">
            <div className="mb-3">
              <Breadcrumbs
                items={[
                  { name: 'Halaman Utama', href: '/' },
            ...
            {/* Shell utama */}
            <section className="rounded-3xl border border-white/10 bg-white/10 backdrop-blur-xl shadow-[0_10px_60px_-15px_rgba(0,0,0,0.6)] ring-1 ring-white/10 px-4 sm:px-6 lg:px-10 py-6 sm:py-7 space-y-6 min-w-0">
              <p className="sr-only">
                Dashboard privat untuk pengguna mengelola antrian, chat, progres, unduhan, dan pembayaran Midtrans.
              </p>
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between min-w-0">
                ...
                {paymentSuccess && (
                  <div className="bg-emerald-500/90 text-white px-4 py-2.5 rounded-2xl shadow-lg ring-1 ring-white/10">
                    🎉 Pembayaran berhasil! Terima kasih.
                  </div>
                )}
              </div>
              ...
              {/* Daftar Tugas */}
              <div className="rounded-2xl bg-white/70 backdrop-blur-md ring-1 ring-slate-200/60 shadow-xl p-4 sm:p-5 min-w-0">
                <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                  <span className="inline-block h-2 w-2 rounded-full bg-blue-600" />
                  Daftar Tugas
                </h2>
                <ul className="mt-3 grid gap-3 sm:gap-4 min-w-0">
                  {userNotifications.map((task: any) => (
                    <li
                      key={task.id}
                      className="group rounded-xl border border-slate-200/70 bg-white/90 shadow-md hover:shadow-2xl transition-all"
                    >
                      <div className="p-4">
                ...
                {userNotifications.map((n: any) => {
                  ...
                  return (
                    <div
                      ...
                      className="cursor-pointer rounded-2xl border border-white/10 bg-white/40 backdrop-blur-md shadow-xl hover:shadow-2xl transition-all"
                    >
                      <div className="p-4 sm:p-5">
                        <p className="font-bold text-blue-700 break-words">📎 {n.fileName}</p>
                        <p className="text-sm text-slate-700 break-words">📝 {String(n.description)}</p>
                        <p className="text-sm text-slate-600">
                          🕒 Deadline:{' '}
                          <span className="font-medium">
                            {new Date(n.deadline).toLocaleString()}
                        ...
                        <p className="text-sm text-slate-600">
                          ...
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-blue-600 underline underline-offset-4 hover:text-blue-700 break-all"
                          >
                            Download File
                          </a>
                        </p>
                              ...
                              {files.map((f, i) => (
                                <li key={i}>
                                  <a
                                    ...
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="text-blue-700 underline underline-offset-4 break-all"
                                  >
                                    {f.fileName}
                                  </a>{' '}
                                  <span className="text-slate-400 text-xs">
              ...
              {showPaymentPopup &&
                ...
                selectedPaymentTask.statusPembayaran !== 'success' && (
                  <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-sm flex justify-center items-center z-50 px-4">
                    <div className="relative bg-white/95 backdrop-blur-xl p-5 sm:p-6 rounded-3xl w-full max-w-[min(90vw,560px)] shadow-2xl ring-1 ring-slate-200/60">
                      <button
                        onClick={() => setShowPaymentPopup(false)}
                        className="absolute top-3 right-3 text-slate-500 hover:text-red-500 text-lg font-bold rounded-full h-8 w-8 flex items-center justify-center hover:bg-slate-100 transition"
                        aria-label="Tutup"
                      ...
                      <h2 className="text-xl font-extrabold text-slate-900 mb-3">Pembayaran Tugas</h2>
                      <p className="mb-1 text-slate-800 break-words">
                        Tugas: <span className="font-semibold">{String(selectedPaymentTask.description)}</span>
                      </p>
                      <p className="mb-1 text-slate-700">
                        Deadline:{' '}
                        <span className="font-medium">
              ...
              <form
                onSubmit={handleGetQueueNumber}
                className="rounded-2xl border border-white/10 bg-white/40 backdrop-blur-md p-4 sm:p-5 shadow-xl space-y-3"
              >
                <label className="block text-slate-900 font-semibold">Masukkan Nama Anda</label>
                <input
                  type="text"
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  placeholder="Masukkan Nama Anda"
                  className="w-full p-3 rounded-xl border border-slate-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/30 bg-white"
                  required
                />
                <button
                  type="submit"
                  className="w-full rounded-xl bg-gradient-to-r from-sky-600 to-cyan-600 hover:from-sky-500 hover:to-cyan-500 text-white font-semibold py-2.5 shadow-lg shadow-cyan-900/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-cyan-500"
                >
                  Dapatkan Nomor Antrian
                </button>
              </form>

              <div className="bg-white/80 backdrop-blur-md p-4 sm:p-5 border border-slate-200/70 rounded-2xl shadow-xl">
                {currentQueueNumber ? (
                  ...
                  <p className="text-base sm:text-lg text-slate-700">Belum ada nomor antrian tersedia.</p>
                )}
              </div>

              {/* Data Worker */}
              <div className="space-y-3">
                <h3 className="font-bold text-lg text-slate-100">Data Worker Terkait</h3>
                {workerProfiles.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {workerProfiles.map((worker) => (
                      <div
                        key={worker.uid}
                        className="group bg-white/80 backdrop-blur-md border border-slate-200/70 rounded-2xl p-4 shadow-lg hover:shadow-2xl transition-all"
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <p className="text-xs font-semibold text-slate-700">No Rekening:</p>
                            <p className="text-xs font-medium text-slate-900 break-all">{worker.noRekening}</p>
                          </div>
                        </div>
                        ...
                        <div className="mt-3 flex items-center gap-1 text-yellow-400">
                          {[1, 2, 3, 4, 5].map((star) => (
                            <FaStar
                              ...
                              onClick={() => handleRating(worker.uid, star)}
                              className={`cursor-pointer transition ${
                                selectedRatings[worker.uid] >= star ? 'text-yellow-400' : 'text-slate-300'
                              }`}
                            />
                          ))}
                        </div>
                        ...
                        <div className="mt-3">
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-blue-700 text-xs font-semibold hover:text-blue-900 transition underline underline-offset-4 break-all"
                          >
                            📂 Lihat Portofolio
                          </a>
                        </div>
                        ...
                        <button
                          onClick={() => handleChooseWorker(worker)}
                          className="mt-3 w-full rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-semibold py-2 shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-blue-500"
                        >
                          Choose
                        </button>
                      </div>
              ...
              {/* Form Kirim Tugas (modal 1) */}
              {selectedWorker && (
                <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md flex items-center justify-center z-50 px-4">
                  <div className="bg-white/95 backdrop-blur-xl p-5 sm:p-6 rounded-3xl w-full max-w-[min(90vw,560px)] shadow-2xl ring-1 ring-slate-200/70">
                    <h2 className="text-lg font-bold mb-3 text-slate-900">Kirim Tugas ke {selectedWorker.name}</h2>
                    <form onSubmit={handleSubmitTask} className="space-y-3">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-slate-800">Deskripsi Tugas</label>
                        <textarea
                          value={description}
                          onChange={(e) => setDescription(e.target.value)}
                          className="mt-1 block w-full p-3 border border-slate-300 rounded-xl focus:border-blue-500 focus:ring-2 focus:ring-blue-500/30 bg-white"
                      ...
                      <button
                        type="submit"
                        disabled={isLoading}
                        className="w-full rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-semibold py-2.5 shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-blue-500 disabled:opacity-70"
                      >
                        {isLoading ? 'Mengirim...' : 'Kirim Tugas'}
                      </button>
                      ...
                      <button
                        type="button"
                        onClick={() => setSelectedWorker(null)}
                        className="w-full rounded-xl bg-slate-700 hover:bg-slate-800 text-white font-semibold py-2.5 transition"
                      >
                        Batal
                      </button>
                    </form>
              ...
              {chatOpen && (
                <div
                  className="fixed bottom-4 right-4 left-4 md:left-auto bg-white/90 backdrop-blur-xl shadow-2xl rounded-3xl p-4 w-auto
                  max-w-[min(92vw,680px)] md:max-w-md
                  h-[75vh] md:h-[70vh]
                  max-h-[85vh]
                  border border-slate-200/70 flex flex-col
                  overflow-hidden z-50 ring-1 ring-white/40"
                >
                  <button
                    onClick={() => setChatOpen(false)}
                    className="flex justify-between items-center border-b pb-2 mb-3 text-slate-500 hover:text-red-500"
                  ...
                  <h2 className="font-bold text-slate-900">Chat with Admin</h2>
                  <div className="overflow-y-auto grow bg-slate-50/80 p-3 rounded-xl border border-slate-200/70 mt-2">
                    {messages.map((msg, index) => (
                      <div key={index} className="mb-3">
                        <div className="p-2 bg-blue-50 text-sm rounded-lg border border-blue-100 text-slate-800 break-words">
                          <p>
                            <strong>{msg.sender}:</strong> {msg.text}
                          </p>
                        </div>
                        ...
                        {msg.type === 'rekomendasi' && (
                          <div className="mt-2 border rounded-xl p-3 bg-white/80 shadow-sm border-slate-200/70">
                            <p className="text-sm font-semibold text-slate-800">Worker yang Direkomendasikan:</p>
                            <div className="flex items-center gap-3 mt-2">
                              ...
                              <div className="min-w-0">
                                ...
                                <a
                                  href={msg.portofolioUrl}
                                  className="text-blue-700 hover:underline text-sm mt-1 inline-block underline-offset-4 break-all"
                                  target="_blank"
                                  rel="noopener noreferrer"
                                >
                                  📁 Lihat Portofolio
                                </a>
                                <p className="text-xs text-slate-600 mt-2 break-words">{msg.description}</p>
                              </div>
                            </div>
                          </div>
                        )}
                    ...
                    <h3 className="font-bold mt-3 text-slate-900">Data Worker Terkait</h3>
                    {workerProfiles.length > 0 ? (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-2">
                        {workerProfiles.map((worker) => (
                          <div
                            key={worker.uid}
                            className="bg-white border border-slate-200 rounded-2xl p-3 shadow-sm hover:shadow-lg transition-shadow"
                          >
                            <div className="flex items-center gap-3">
                              <img
                                src={worker.profilePictureUrl}
              ...
              {/* Form Kirim Tugas (modal 2 – tetap dipertahankan sesuai kode asli) */}
              {selectedWorker && (
                <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md flex items-center justify-center z-50 px-4">
                  <div className="bg-white/95 backdrop-blur-xl p-5 sm:p-6 rounded-3xl w-full max-w-[min(90vw,560px)] shadow-2xl ring-1 ring-slate-200/70">
                    <h2 className="text-lg font-semibold mb-3 text-slate-900">Kirim Tugas ke {selectedWorker.name}</h2>
                    <form onSubmit={handleSubmitTask} className="space-y-3">
                      <div>
                        <label className="block text-sm font-medium text-slate-800">Upload File</label>
                        <input
      ...
      {/* Bungkus Footer agar tidak memicu overflow jika ada w-screen di dalamnya */}
  ...
  <Footer />
      ...
      {/* Global guard tambahan (aman, tidak mengubah fungsi apa pun) */}
      <style jsx global>{`
        html,
        body,
        #__next {
        ...
        /* jaga elemen yang suka melebar (img/link/preelements) */
        img,
        video,
        canvas,
        svg {
